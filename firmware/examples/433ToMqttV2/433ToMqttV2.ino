...
const unsigned long debounceDelay = 1000;  // 3 seconds debounce
...
void blinkLED(int times) {
  for (int i = 0; i < times; i++) {
    ...
    delay(150);  // Wait for 50 milliseconds
...
void handleAction(const String &action) {
  ...
  blinkLED(2);  // Blink the LED twice when an action is handled
}
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  ...
  blinkLED(2);  // Blink the LED twice when a message is received
...
void setup() {
  ...
  // Set up ticker for checking RF signals
  rfCheckTicker.attach_ms(100, checkRFSignal);  // Check every 100 ms

  if (DEBUG) {
    Serial.println("Setup complete. Waiting for MQTT messages and RF signals...");
  }
}
